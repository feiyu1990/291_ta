function [opttheta] = vwfa_mine_minFuncSGD(funObj,theta,train_images,train_labels, imageDim, ...
                        options,ei)
...
for e = 1:epochs
    ...
    rp = randperm(floor((m-minibatch+1)/minibatch));
    ...
    for s=1:length(rp)
        ...
        [mb_data, mb_labels] = load_data(train_images, train_labels, minibatch, rp(s), imageDim);
        %disp(size(mb_data));
        %disp(size(mb_labels));
        ...
        %if (mod(it, 20) == 0)
        %    [~,cost,preds]=mine_cnnCost(ei, theta,data_test,label_test,numClasses,...
        %        true);
        %    acc = sum(preds==label_test)/length(preds);
        %    fprintf('Test accuracy: %f',acc);%
        %end
    end;
