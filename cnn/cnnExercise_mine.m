...
images = randn(imageDim,imageDim,numInput, numImages);
%images = reshape(images,imageDim,imageDim,numImages,1);

W = randn(filterDim,filterDim,numInput,numFilters);
b = rand(numFilters,1);
...
convolvedFeatures = cnnConvolve_mine(filterDim, numFilters, convImages, W, b);
...
for i = 1:1000   
    ...
    feature = sum(sum(sum(patch.*W(:,:,:,filterNum))))+b(filterNum);
...
pooledFeatures = cnnPool(poolDim,poolDim, convolvedFeatures,'mean');
...
expectedMatrix = [max(max(testMatrix(1:4, 1:4))) max(max(testMatrix(1:4, 5:8))); ...
                  max(max(testMatrix(5:8, 1:4))) max(max(testMatrix(5:8, 5:8))); ];
            
testMatrix = reshape(testMatrix, 8, 8, 1, 1);
        
pooledFeatures = squeeze(cnnPool(4,4, testMatrix,'max'));
