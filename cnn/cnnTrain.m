...
theta = cnnInitParams(imageDim,filterDim,numFilters,numChannels, poolDim,numClasses);
...
if DEBUG
    ...
    db_numChannels = 1;
    db_filterDim = ei.layer_param{1}.filterDim;
    db_poolDim = ei.layer_param{3}.pool_size;
    db_images = images(:,:,:,1:10);
    db_labels = labels(1:10);
    db_theta = cnnInitParams(imageDim,db_filterDim,ei.layer_param{1}.numFilters,...
                db_numChannels, db_poolDim,numClasses);
    
    [cost grad] = cnnCost(ei, db_theta,db_images,db_labels,numClasses);
    

    % Check gradients
    numGrad = computeNumericalGradient( @(x) cnnCost(ei,x,db_images,...
...
opttheta = minFuncSGD(@(x,y,z) cnnCost(ei, x,y,z,numClasses),theta,images,labels,options);
...
[~,cost,preds]=cnnCost(ei, opttheta,testImages,testLabels,numClasses,...
                true);
