...
train.X = [ones(1,size(data.train,2)); data.train]; 
test.X = [ones(1,size(data.test,2)); data.test];
train.y = data.train_label'; % make labels 1-based.
test.y = data.test_label'; % make labels 1-based.

num_classes = length(unique(train.y));
% Training set info
m=size(train.X,2);
n=size(train.X,1);
