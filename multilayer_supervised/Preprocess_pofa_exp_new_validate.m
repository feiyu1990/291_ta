function [  ] = Preprocess_pofa_exp_new_validate( name ,name_validate, num_pca)
...
for i =1:length(label)
    ...
    if strcmp(temp(1:2) , name_validate)
        train_indice(i) = 2;
    end
end
...
label_validate = label(train_indice==2);


total_train = sum(train_indice==1);
total_test = sum(train_indice==0);
total_validate = sum(train_indice==2);

f_filtered_normalized_dwsp_vector_allsub_train = zeros(8*8*s*o, total_train);
f_filtered_normalized_dwsp_vector_allsub_validate = zeros(8*8*s*o, total_validate);
...
for i=1:length(files)
    ...
    elseif train_indice(i) == 1
        ...
        f_filtered_normalized_dwsp_vector_allsub_validate(:,validate_i)=f_filtered_normalized_dwsp_vector(:);
        %f_filtered_normalized_dwsp_vector_allsub_train(:,end+1)=f_filtered_normalized_dwsp_vector;
...
f_filtered_normalized_dwsp_vector_allsub_validate=bsxfun(@minus, f_filtered_normalized_dwsp_vector_allsub_validate, m);
f_filtered_normalized_dwsp_vector_allsub_validate=bsxfun(@rdivide, f_filtered_normalized_dwsp_vector_allsub_validate, s+.1);



data.train = f_filtered_normalized_dwsp_vector_allsub_train;
data.validate = f_filtered_normalized_dwsp_vector_allsub_validate;
data.test = f_filtered_normalized_dwsp_vector_allsub_test;
data.train_label = label_train;
data.validate_label = label_validate;
data.test_label = label_test;
a = strcat(name,'_validate_POFA_GaborData_expression.mat');
save(a{1},'data');
...
for scales=1:size(k,2) 
    ...
    scale_all_validate=f_filtered_normalized_dwsp_vector_allsub_validate((scales-1)*dwsp*dwsp*size(phi,2)+1:scales*dwsp*dwsp*size(phi,2),:);
    ...
    mean_subst_validate=scale_all_validate-repmat(mean_images,1,total_validate);
    ...
    temp = vector_ori'*mean_subst_validate;
    temp=bsxfun(@minus, temp, m);
    f_PCA_scale_validate_normal=bsxfun(@rdivide, temp, s);
...
%         f_PCA_scale_test_normal=(vector_ori(:,:,scales)'*mean_subst_test(:,:,scales));        
    ...
    f_PCA_validate_temp_normal((scales-1)*num_pca+1:scales*num_pca,:)=f_PCA_scale_validate_normal;
end


data.train = f_PCA_temp_normal;
data.test = f_PCA_test_temp_normal;
data.validate = f_PCA_validate_temp_normal;
data.train_label = label_train;
data.validate_label = label_validate;
data.test_label = label_test;


a = strcat(name,'_validate_POFA_PreprocessedData_expression_',int2str(num_pca),'.mat');
save(a{1},'data')

train_image = files(train_indice == 1);
test_image = files(train_indice == 0);
validate_image = files(train_indice == 2);

information.train= train_image;
information.validate= validate_image;
information.test = test_image;
a = strcat(name,'_validate_POFA_expression_info.mat');
