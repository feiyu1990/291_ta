...
root = strcat(pwd, '/../../programming/PA3/faces/');
...
for i=1:length(files)
    identities = [identities; files{i}(1:2)];
end
identities_unique = unique(identities);
for i=1:length(identities)
    [m,label(i)] = ismember(identities{i},identities_unique);
end
...
save(['POFA_GaborData_mine.mat'],'data')
...
save(['POFA_PreprocessedData_orig.mat'],'preprocessedData')
save(['POFA_PreprocessedData_mine.mat'],'data')
...
save('POFA_Image_info.mat','information');
